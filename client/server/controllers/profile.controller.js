...
exports.createProfile = async (req, res) => {
    ...
    try {
        ...
        if (!userExists) {
            return res
                .status(400)
                .json({ error: 'Пользователь не существует' });
        }
        ...
        res.status(500).json({
            error: `Ошибка создания профиля: ${err.message}`,
        });
    }
};
...
exports.getProfileById = async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({
            error: `Ошибка получения профиля: ${err.message}`,
        });
    }
};
...
exports.deleteProfile = async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({
            error: `Ошибка удаления профиля: ${err.message}`,
        });
    }
};
