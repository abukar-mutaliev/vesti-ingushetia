...
exports.createNewsValidator = [
    ...
    .isLength({ min: 20 }).withMessage('Содержание должно содержать не менее 20 символов'),
    body('categoryId')
    .notEmpty().withMessage('ID категории обязателен')
...
exports.updateNewsValidator = [
    param('id')
    .isInt().withMessage('ID новости должен быть числом'),
    body('title')
    .optional()
    .isLength({ min: 5 }).withMessage('Заголовок должен содержать не менее 5 символов'),
    body('content')
...
exports.getNewsByDateValidator = [
    ...
    .isISO8601().withMessage('Дата должна быть в формате ISO8601'),
];
