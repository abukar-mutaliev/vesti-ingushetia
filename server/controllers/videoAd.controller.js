...
exports.createVideoAd = async (req, res) => {
  try {
    ...
    const videoUrl = path.posix.join("uploads", "videoAd", videoFile.filename);
...
exports.updateVideoAd = async (req, res) => {
  try {
    ...
    if (req.file) {
      const oldVideoPath = path.join(__dirname, "..", "uploads", videoAd.url);
      ...
      const videoUrl = path.posix.join("uploads", "videoAd", req.file.filename);
...
exports.pauseVideoAd = async (req, res) => {
  try {
    ...
    res.status(200).json({ message: "Видеореклама приостановлена", videoAd });
  } catch (error) {
    res.status(500).json({ error: "Ошибка сервера: " + error.message });
  }
};
