...
exports.createProfile = async (req, res) => {
    ...
    if (!userId) {
        return res.status(400).json({ error: 'Не указан пользователь для профиля' });
    }
    ...
    try {
        ...
        const profile = await Profile.create({ userId, avatarUrl, bio });
...
exports.updateProfile = async (req, res) => {
    ...
    try {
        ...
        await profile.update({ avatarUrl, bio });

        res.status(200).json({ message: 'Профиль успешно обновлен', profile });
    } catch (err) {
        res.status(500).json({ error: `Ошибка обновления профиля: ${err.message}` });
    }
};
