const { Project, Category, Media, sequelize } = require("../models");
...
exports.getAllProjects = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: `Ошибка получения проектов: ${err.message}` });
  }
};
...
exports.getProjectById = async (req, res) => {
  try {
    ...
    res.status(500).json({ error: `Ошибка получения проекта: ${err.message}` });
  }
};
...
exports.createProject = async (req, res) => {
  ...
  try {
    ...
    const project = await Project.create({ title, content }, { transaction });
...
exports.updateProject = async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      res
        .status(200)
        .json({ message: "Проект успешно обновлен", project: modifiedProject });
    } catch (err) {
      if (transaction) await transaction.rollback();
      res
        .status(500)
        .json({ error: `Ошибка обновления проекта: ${err.message}` });
    }
  } catch (err) {
...
exports.deleteProject = async (req, res) => {
  try {
    ...
    res.status(500).json({ error: `Ошибка удаления проекта: ${err.message}` });
  }
};
